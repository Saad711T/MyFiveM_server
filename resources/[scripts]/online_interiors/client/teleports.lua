...
--	table.insert(INTERIORS,{text = "Los Santos", dest = {#INTERIORS + 2}, x=935.0, y=46.0, z=81.1, h=109.0})
--	table.insert(INTERIORS,{text = "Diamond Casino", dest = {#INTERIORS}, x=1090.5, y=207.8, z=-49.0, h=4.0})
--end
...
local function teleport(inter)
	local ped = GetPlayerPed(-1)
	if(IsPedInAnyVehicle(ped))then
		if inter.veh == false then
...
local function gui_interiors_drawMenuButton(button,x,y,selected)
	...
	SetTextEntry("STRING")
	if button ~= nil then
		AddTextComponentString(button.text)
	end
	if selected then
...
local function getplyorvehpos(ped)
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(500)
		local playerPos = getplyorvehpos(GetPlayerPed(-1))
		torender = {}
		POS_actual = 0
		for id,inter in pairs(INTERIORS) do
			local pos = inter.coord
			if pos == nil then
				print(inter.text .. " is missing coords!")
				INTERIORS[id] = nil
			else
				local distance = Vdist2(playerPos.x, playerPos.y, playerPos.z, pos.x, pos.y, pos.z)
				if distance < 62500 then
					table.insert(torender,id)
					if distance < 2 then
						POS_actual = id
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(0)
		for _,id in pairs(torender) do
			local pos = INTERIORS[id].coord
			DrawMarker(1, pos.x, pos.y, pos.z-1.02, 0, 0, 0, 0, 0, 0, 0.7,0.7,0.8, 255,255,255, 200, 0, 0, 2, 0, 0, 0, 0)
		end
		if POS_actual == 0 then gui_interiors_CloseMenu() end
		if gui_interiors.opened then
			...
			gui_interiors.lastbuttoncount = 0
			if gui_interiors.currentmenu == "main" and INTERIORS[POS_actual] and INTERIORS[POS_actual].dest then
				for i,btn in pairs(INTERIORS[POS_actual].dest) do
